...
public class EntityManagerGeneratorTests {
    ...
    void testIsEntityManagerFactorySingletonInstance() {
        ...
        Assertions.assertEquals(factory1, factory2); // 주소값 동일 여부 판단
    }

    /* 필기
        엔티티 매니저(EntityManager) 란?
        엔티티 매니저는 엔티티를 저장하는 메모리상의 데이터베이스를 관리하는 인스턴스이다.
        엔티티를 저장하고, 수정, 삭제, 조회하는 등의 엔티티와 관려뇐 모든 일을 한다.
        엔티티 매니저는 스레드 세이프하지 않기 때문에 동시성 문제가 발생할 수 있다.
        따라서 스레드 간 공유를 하지 않고, web의 경우 일반적으로는 request scope와 일치한다.

        영속성 컨텍스트(PersistenceContext) 란?
        엔티티 매니저를 통해 엔티티를 저장하거나 조회하면 엔티티 매닞러는 영속성 컨텍스트에 엔티티를 보관하고 관리한다.
        영속성 컨텍스트는 엔티티를 key value 방식으로 저정하는 저장소이다.
        영속성 컨텍스트는 엔티티 맨지러를 생성할 때 하나 만들어진다.
