...
public class EntityLifeCycleTests {
    ...
    필기.
        ...
        컨텍스트 별로 객체를 공유하지 못함. 각자 따로의 라이프사이클을 가지고 있음.
        엔티티 매니저가 엔티티 객체를 저장하는 공간으로 엔티팉 객체를 보관하고 관리한다.
        (엔티티 매니저가 생생될 때 하나의 영속성 켄텍스트가 만들어진다.)


        엔티티 생명주기
        비영속 (new/transient) : 영속성 컨텍스트와 전혀 관계가 없는 상태. (식별자가 존재하지 않을 수 있다.)
        영속 (managed) : 영속성 컨텍스트에 저장된 상태
        준영속 (detached) : 영속성 컨텕스트에 저장되었다가 분리된 상태
        삭제 (removed) : 삭제
   */
    ...
    void testManagedNewEntity(String menuName, int menuPrice, int categoryCode, String orderableStatus) {
        ...
        transaction.begin();
        manager.persist(menu); // 영속성 생성 SQL 문 호출 안됨.
        manager.flush();
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        //when
        transaction.begin();
        manager.detach(foundMenu); // 준영속상태로 변경
